...
class SceneCatalogEntry:
    ...
    @property
    def thumbnail_video(self) -> VideoVariant | None:
        """Video to grab the card THUMBNAIL from. Prefers a standard-aspect
        render — an ultrawide/cropped frame makes a poor, letterboxed thumbnail —
        falling back to the default pick when only aspect variants exist."""
        for v in self.videos:
            if not v.is_aspect_variant:
                return v
